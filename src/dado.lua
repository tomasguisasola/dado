...
local tunpack = require"table".unpack

local assert, error, pcall, require, setmetatable, tostring, type, xpcall
	= assert, error, pcall, require, setmetatable, tostring, type, xpcall
...
function M.connect (dbname, dbuser, dbpass, driver, ...)
	...
	if not conn then
		...
		if not ok then
			error ("Could not load LuaSQL driver `"..driver
				.."'. Maybe it is not installed properly.\nLuaSQL: "..tostring(luasql))
		end
